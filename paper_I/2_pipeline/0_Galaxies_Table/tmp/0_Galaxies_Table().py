...
with open( '../../0_data/tmp/Chosen_FCC_FDS.csv', 'r' ) as f:
    name = csv.reader(f)
...
for l in range(size):
	...
	for j in range(len(tbdata_FDS.field('Target'))):
		if tbdata_FDS.field('Target')[j]==name_FDS:
			...
			M_r[l] = tbdata_FDS.field('r_mag')[j] - 31.51
			ERR_m_r[l] = tbdata_FDS.field('r_mag_e')[j]
			M_g[l] = tbdata_FDS.field('g_mag')[j] - 31.51
...
for l in range(size):
	draft = name_list[l]
	m_r = M_r[l] + 31.51
	nameFDS = 'FDS'+draft[1]+'_DWARF'+"{0:0=3d}".format(int(draft[2]))
	nameFCC = 'FCC'+draft[0]
	dict={'FCC name':nameFCC, 'FDS name':nameFDS, 'year': draft[3], 'RA (deg)':"%.6f"%RA[l], 'DEC (deg)':"%.6f"%DEC[l],
			'm_r (mag)':"%.2f" % m_r, 'M_r (mag)':"%.2f" % M_r[l], 'ERR_m_r (mag)':"%.2f" % ERR_m_r[l],
			'mu_r (mag/arcsec2)':"%.2f" % mu_r[l], 'ERR_mu_r (mag/arcsec2)':"%.2f" % ERR_mu_r[l] , 
			'R_e (arcsec)':"%.2f" % R_eff[l], 'ERR_R_e (arcsec)':"%.2f" % ERR_R_eff[l] , 
			'axis ratio':"%.2f" % axis_ratio[l], 'Sersic index':"%.2f" % sersic_index[l],'log10(M_*/M_Sun)':"%.4f" % log_mass[l], 'ERR_log10(M_*/M_Sun)':"%.4f" % ERR_log_mass[l],
			}
	ListDict.append(dict.copy())

with open('./Chosen_Galaxies_Table.csv', 'w') as output_file:
